...
## 4.8 お問い合わせフォーム（/contact）

**2026-09-01 追加。連絡の主窓口をXからフォームへ移した**（本人の指示）。
**Xは消していない。二段構えにしただけ。**

| 用途 | 窓口 |
|---|---|
| お問い合わせ（感想・不具合・権利・仕事） | **`/contact` のフォーム** |
| 軽い連絡・交流 | X |

| どこ | 何が入っているか |
|---|---|
| `src/data/site.ts` の `CONTACT` | **正本。** About・目次・プレスキットはここを参照している |
| `src/data/site.ts` の `CONTACT_TOPICS` | 種別5つ。**`functions/api/contact.js` の `TOPICS` と対** |
| `src/pages/contact.astro` | フォーム本体。送信は委譲＋`astro:page-load`（4.7と同じ作り） |
| `src/pages/privacy.astro` | 個人情報の扱い。**フォームがある以上、外せない** |
| `functions/api/contact.js` | 受け口。D1 は plays / views / nazo と同じ `DB` |
| `tools/inbox-db.mjs` | 読み書きの共通部分。**wrangler を叩くのはここだけ** |
| `tools/inbox.mjs` ／ `tools/inbox-app.mjs` | 端末版 ／ 画面版。`npm run inbox` ／ `inbox:app` |
| `tools/inbox-app.cmd` ＋ `inbox.ico` | デスクトップの「お問い合わせ受信箱」の中身 |

> [!WARNING] 触るときに壊してはいけないもの
> - **`offline` を「送信しました」と表示しない。** D1が無いときは本文がどこにも残らない。
>   そこで礼を出したら、**「動いてないものの計上」そのもの**になる（ユーザーメモリ5節）
> - **`ctMore`（もう1件送る）で `opened` を測り直さない。** 測り直すと、2件目を素早く
>   送った人が「速すぎる送信」の網に掛かり、**本文が消えたまま礼だけ出る**
> - **知らない種別を捨てない。** `other` へ倒す。**取りこぼすより誤ラベルのほうが軽い**
> - **管理画面を作らない**（5節）。ここで禁じているのは**サイト上に置く**読み取り口のこと。
>   `npm run inbox:app` は `127.0.0.1` にしか待ち受けず、公開サイトには1バイトも足していない。
>   **`0.0.0.0` へ変えない／起動ごとの合言葉（token）を外さない。**
>   合言葉を外すと、本人が見ている**別のWebページから `127.0.0.1` 経由で中身を読める**
> - **`npx` を spawn しない**（2026-09-02 に踏んだ）。Windows では `npx.cmd` に `shell:true` が
>   要り、**空白を含む引数＝SQL文がシェルに分解される。**
>   `node node_modules/wrangler/bin/wrangler.js …` と実体を直に叩く
> - **`tools/inbox-app.cmd` に日本語を書かない。** cmd はコードページで解釈するので壊れる。
>   窓のタイトルは Node 側（`process.title`）から付けている
> - **`privacy.astro` は実装と一致していなければならない。** IPを保存し始める、
>   アクセス解析を入れる、Cookieを使う——どれをやってもあのページは嘘になる。
>   **コードを変えたら本文も直す**（根拠は `privacy.astro` の冒頭コメントに列挙してある）
> - **メールアドレスをリポジトリに書かない**（5節）。フォームにしたのは、この制約を
>   保ったまま「Xを持っていない人」から受け取るため

- 迷惑メール対策は**静かに捨てる**で統一（蜜壺・3秒未満・1日12件超）。
  **捨てたことを相手に教えない。** 教えると条件を1つずつ外して通るまで試せる
- **Turnstile（CAPTCHA）は入れていない。** サイトキーと秘密鍵の設定が要り、
  「Xを持っていない人でも気軽に送れる」という導入の目的と正面から競合する。
  実際に迷惑メールが通り始めたら検討する
- 種別「権利関係について」は消さないこと。二次創作を出している以上、
  **連絡できる窓口が明示されていること自体が中身**になる
...
- **平文のメールアドレスを載せる**（スパムに拾われる。**この制約は生きている**）
  → 連絡先そのものは 2026-09-01 に「Xのみ」から**フォーム＋Xの二段構え**へ変わった（4.8節）。
  変わったのは窓口であって、**メールアドレスを書かない**という判断ではない
- 巨大JSライブラリ・3Dページめくり（仕様書10章・21章）
- 会員登録 / コメント / CMS / 管理画面（仕様書24章）
  → **お問い合わせは受け口だけ足した**（4.8節。plays / views / nazo と同じ Pages Functions）。
  **管理画面は作っていない。** 読むのは `npm run inbox` で、本人のPCからしか触れない
...
npm run inbox      # 届いたお問い合わせを端末で読む（4.8節。初回は npx wrangler login）
npm run inbox:app  # 同じものを画面で読む。デスクトップのアイコンはこれを起動している
```
...
- ~~連絡は **X のみ**~~ → **2026-09-01 変更。フォーム（`/contact`）が主窓口、Xは軽い連絡用**（4.8節）。
  `src/data/site.ts` の `CONTACT` が正本。About・目次・プレスキット・柱はここを参照している。
