...
> [!IMPORTANT] **表紙に出す作品は3本まで**（本人・2026-09-04）
> 「追加したコンテンツ、トップに出すのは3つくらいにしようと思う」
>
> 作品が7本になった時点で、表紙の見出しも7本になっていた。**足すのは簡単で、
> 減らす理由が誰にも無かった**ので溜まった。上限をルールとして書いておく。
>
> | | |
> |---|---|
> | 選び方 | **ジャンルの代表を1本ずつ**（ツール・アプリ／ゲーム／TRPGシナリオ） |
> | 残りはどこで見せるか | **目次のジャンル欄**（`/contents`）。0件のジャンルは欄ごと出ない |
> | 足したくなったら | **まずどれかを外す。** 4本目を足さない |
> | 新作をどう見せるか | **`headlines` に書かない。** 左の **NEW WORKS 欄**が公開日の新しい順で自動的に出す |
>
> ⚠️ **特集見出しと NEW WORKS を重複させない。** ここは「特集＝代表作」、新作欄は「新作」。
> 777コンボを見出しに入れたら、**一冊の刷り見出し・特集見出し・NEW WORKS で3回**出た（2026-09-04）。
>
> **表紙・目次・作品ページの並びと通し番号は `src/lib/features.ts` が正本。**
> **番号と並びは別物**（本人・2026-09-05「番号がばらばらだから、ちゃんと出した順にしてほしい」）：
>
> | | 何で決まるか |
> |---|---|
> | **番号**（FEATURE 01…） | **出した順**＝公開日の古い順。**新作が最大の番号を取り、既存の番号は動かない** |
> | **表示の並び** | ジャンル順 → ジャンル内は公開日の**新しい順** |
>
> `headlines` で **`no:` を省くと、その番号が自動で入る**（手で振り直さない）。
...
3. `src/content/works/<slug>.md` を作る（必須項目は §3）
   → **`category` がそのまま目次のジャンル欄になる。** ここを間違えると別の欄に並ぶ
4. **`headlines` には足さない。** 公開日がいちばん新しいので、表紙の
   **NEW WORKS 欄**（左カラム・新しい順3件）が自動で拾う
   → 代表作として**特集**に据えたいときだけ `issues/vol01.md` の `headlines` へ。
     ⚠️ **表紙の特集は3本まで**（§0）。足すなら**先にどれかを外す。**
     **NEW WORKS と重複させない。** `no:` は書かない（目次の通し番号が自動で入る）
5. `npm run build` を通す（**通らないものを報告しない**）
6. 差分を見せてから commit

### 「小物を1件足して」／「これ作ったから載せて」（作品ページを作らないもの）

**`src/content/bits/<slug>.md` を1枚置くだけ。** 画像も本文も要らない。

```yaml
---
title: 動くクリティカルカットイン素材
date: 2026-09-04      # **棚に出した日**。配布開始日が分からないときは今日でよい
catch: ココフォリアで決定的成功が出たときに走るカットイン。GIFと効果音のセット。
url: https://tiiinnstudio.booth.pm/items/8636341   # 省略可（まだ配っていないもの）
priceType: free       # 省略可。**書かなければ何も表示しない**（推測で埋めない）
tags: [TRPG, ココフォリア, 素材]
---
```

配布元の名前（BOOTH / GitHub / …）は **URL から機械が出す**ので書かない（`src/lib/bits.ts`）。
出る場所は **`/bits`（小物）** と、目次の `SMALL WORKS ／ 小物` 欄（新しい順に5件まで）。

> [!IMPORTANT] **この欄を重くしない**（本人・2026-09-04）
> 「小さいこまごまとしたもの作りました〜みたいなのを**簡単に**公開できる場所」が要件。
> 画像必須・本文必須・個別ページを足した瞬間、また溜まる。
> **育ったら `works/` へ引っ越す**（そのとき初めて画像と本文を書く）。

NEWSでも告知するなら `url: https://ogyanuntiusxiii.com/bits#<slug>` にする。
**棚を経由してから配布元へ出る**ので、ほかの小物も目に入る（`newsLink.ts` が内部リンクへ落とす）。
...
> | `url` に `<origin>/bits#<slug>` | **小物**（`src/content/bits/`） | 棚の該当行 → そこから配布元 |
> | 両方 | — | **ビルドが落ちる** |
>
...
1. `src/content/works/<slug>.md` の `status` を `published` に、`date` を公開日に
2. `src/content/news/` に `kind: release` を1件足す
3. **表紙の `headlines` には足さない。** 公開日がいちばん新しくなるので、
   表紙の **NEW WORKS 欄**が自動的に拾う（§0）

> [!NOTE] **NOW MAKING（制作中）の欄は 2026-09-04 に廃止した**
> 本人「**NOW MAKING そのものをもうサイトから消しちゃって、新作の並び順で出しとこうかな**」。
> `nowmaking` コレクション・`src/content/nowmaking/` ごと消してある。
> **表紙に出るのは公開したものだけ**になった。復活させるなら本人に確認する。
...
**`no:` は書かない。** 省くと `src/lib/features.ts` の通し番号（＝目次の番号）が入る。
**表紙に出す作品は3本まで**（§0）。

### `src/content/bits/*.md` — 小物（SMALL WORKS）

作品ページを作るほどではないもの。**わざと軽い**（画像・本文・description すべて不要）。
書き方は §2 の「小物を1件足して」。出る場所は `/bits` と、目次の `SMALL WORKS ／ 小物` 欄。

### `src/lib/features.ts` — **並びと通し番号の正本**

表紙・目次・作品ページが、それぞれ勝手に並べ替えていた。その結果
**同じ作品が目次では 01、作品ページでは FEATURE 02** になっていた（2026-09-04 に一本化）。

| | |
|---|---|
| 番号 | **出した順**（公開日の古い順）。01＝いちばん最初に出したもの。**並び順から番号を作らない** |
| 表示の並び | ジャンル順（`content.config.ts` の `category` と同じ順）→ ジャンル内は**公開日の新しい順** |

| ジャンル欄のアンカー | `/contents#cat-<category>`。作品ページの「← 目次へ戻る」がここへ着地する |
| `resolve(ref, where)` | **解決できないとビルドを落とす。** 表紙 `headlines` と NEWS の `ref` は全部ここを通す |
| `latest` | ジャンルを無視した**公開日の新しい順**。表紙の NEW WORKS 欄がこの先頭3件を出す |

### `src/data/site.ts` の `NAV` — **行き先の正本**

柱（ヘッダー）とフッターが**この1つ**を読む。2026-09-05 まで、行き先の集合が
**ヘッダー2つ／フッター5つ／表紙6つ**とバラバラで、**`/bits`（小物）は表紙からしか行けなかった。**

- `primary: true` の項目だけが柱に出る。フッターは全部＋「表紙へ戻る」
- いま開いているページには `aria-current="page"` が付く（赤字＋下線。**色だけで示さない**）
- ⚠️ **表紙（`index.astro` の `cover__foot`）はここを読まない。** 表紙は載せる数を絞る場所（§0）。
  **ページを1枚足したら `NAV` と表紙の両方を見る。**

### 作品ページの「ほかの◯◯」

記事の最後に**同じジャンルのほかの作品**を出す（`WorkArticle.astro` の `siblings`）。
これが無いと1本読んだところで行き止まりになり、目次へ戻らないと隣へ行けなかった。
`/works/[slug].astro` と `/scenario/[slug].astro` が `features.groups` から作って渡す。

⚠️ **ページ側で `/works/${ref}` を組み立てないこと。**
`ref` がシナリオを指した瞬間、黙って404へ飛ぶリンクになる（廃止した NOW MAKING が実際にそうだった）。
...
### 通知（Discord Webhook）— 2026-09-02 に繋がった

`CONTACT_WEBHOOK`（Pages のシークレット）が入っていると、1件受けるたびに
**「届いた」とだけ**投げる。**本文は送らない**（`/privacy` の「第三者へ渡さない」を守るため）。
飛んだかどうかは **`npm run inbox -- --ops`** で読む。

> [!WARNING] ここで半日溶かした。**同じ轍を踏まないための3つ**
> | 踏んだ罠 | どうなるか | 正しいやり方 |
> |---|---|---|
> | **シークレットを変えただけで再デプロイしない** | **既存のデプロイには反映されない**（ビルド時に焼き込まれる）。入れ直しても古い値のまま動き続ける | 入れ直したら**必ずデプロイし直す**（空コミットでよい） |
> | **プロンプトへ `Ctrl+V` で貼る** | 貼り付けではなく**キー入力そのもの（U+0016）が1文字だけ保存される**。`Invalid URL` で静かに落ちる | **右クリック**か `Ctrl+Shift+V`。Enter前に **`*` の数**を見る。1個なら失敗 |
> | **通知を `waitUntil` に載せる** | Discordにも届かず、**D1の記録にも1行も残らない**。応答後の処理が走っていない | **`await` して返す。** 数百ミリ秒より、原因の分からない沈黙のほうが高くつく |
>
> - **例外メッセージをそのまま記録しない。** `Invalid URL: <値>` の形で
>   **シークレットの中身がD1に残った**（実際に残った。削除済み）。残すのは種類と「形」だけ
> - `wrangler pages deployment tail` は**このプロジェクトでは使えない**
>   （「このデプロイには Function が無い」と言って拒む。実際には動いている）。
>   **だから `ops` 表に書いて読む。** この経路を消さないこと
...
## 4.9 数える仕組み（**人** と **総PV**）— 2026-09-06 に2本立てにした

**表紙に「本日 12人 34PV／累計 340人 1,204PV」の形で出る。** 出どころは
`functions/api/views.js`（D1 の `views` 表）と `src/layouts/Base.astro` の末尾のビーコン。

| 数 | 何の数 | 増える条件 | D1 の鍵 |
|---|---|---|---|
| **人** | 来た人（延べ） | **1つのブラウザにつき1日1回。ゲームのページも含む** | `site` ／ `day:YYYY-MM-DD` |
| **総PV** | ページを見た回数 | **表示のたび1。サイト内の移動・ゲームも含む** | `pv` ／ `pv:day:YYYY-MM-DD` |

> [!WARNING] **2026-08-27〜09-05 の数字は Cloudflare Web Analytics から入れたもの**
> 自前で数え始めたのは 09-06。**同じ日を二重に数えてはいない**（加算ではなく期間で連結）。
> どこまでが外部集計かは D1 自身が持っている（`meta:pv-backfill-through` / `meta:people-backfill-through`）。
> 09-05 以前の日別行を**上書きし直さないこと。**
> 旧「表紙に来た人だけ」の記録は `cover:site` / `cover:day:*` に退避してある。**消さない。**

```
GET  /api/views      読むだけ
POST /api/views      総PV +1
POST /api/views?v=1  人 +1 と 総PV +1（今日まだ数えていない人）
```

返す形は `{ people:{today,total}, pv:{today,total}, today, total }`。
末尾の `today` / `total` は **people の別名**（古いHTMLが残っていても人の数だけは出る）。

> [!WARNING] **人とPVを混ぜて出さない**
> PVを訪問者数として見せたら、それは誇張。**必ず並べて出す**（本人・2026-09-06
> 「人と、総PVと出しときたい」）。「1人あたり◯PV」を出すなら、元の2つも一緒に出す。

### 触るときの決まり

- **鍵は外から受け取らない。** 使うのは上の4つだけで、全部サーバ側で作る。
  **ページ別PVが要るようになったら、先に「許可するパスの一覧」を持つこと。**
  パスをそのまま鍵に混ぜると、好きな名前でゴミ行を無限に積める
- **「今日」は日本時間で切る**（サーバもブラウザも）。UTCだと日付の変わり目が朝9時になる
- **`public/games/*` と `public/tools/*` は Astro を通らない。** 各 `index.html` の `</body>`
  直前に置いたビーコンが撃っている。**ゲームやツールを増やしたら、その塊ごとコピーする。**
  入れ忘れたページは数に乗らない。
  **鍵（`ogyanun.viewed`）と日付の切り方は `Base.astro` と必ず揃える。**
  ここだけ変えると、表紙とゲームで同じ人を2回数える
- ビーコンは **公式ドメインのときだけ**撃つ（`tools/pack-game.py` で zip にして
  よそへ出したぶんは何もしない）
- 書き込みは PVだけなら1回2行、人も数える回は4行。無料枠は1日10万行なので
  **1日およそ2.5万PVまで無料。** 超えてもエラーになるだけで課金は起きない。
  **Workers Paid へ切り替えないこと**
- **`/privacy` の本文は、この仕組みと一致していなければならない。** 数え方を変えたら本文も直す

> [!WARNING] **1回の表示で2回撃たない**（2026-09-06 に直した）
> ClientRouter は初回表示のときに `astro:page-load` を `window` の `load` で発火させる。
> ビーコンを「その場の呼び出し＋`astro:page-load`」の両方に置くと、**初回だけ2発**出る。
> `armed` フラグで1回に絞り、`astro:before-preparation`（遷移の開始）で戻している。
> 人の印（`localStorage`）も**送る前に**立てる。あとで立てると2発が競り合う。

## 4.10 ゲームのJSを更新するとき — **モジュールの新旧混在で止まる**（2026-09-12 に本番で踏んだ）

Cloudflare Pages は `.js` を **`Cache-Control: max-age=14400`** で配る。`index.html` の入口
（`game.js?v=…`）だけ版を変えても、そこから `import` される `world.js` / `flight.js` は
**以前に遊んだブラウザでは4時間は古いまま**使われる。『Bの意地』第2版（`906706b`）では
新しい `game.js` が古い `flight.js` の状態を回して最初の物理計算で例外を出し、
**「飛び立った瞬間に止まる」**になった（本人報告）。手元の dev サーバーでは常に新品が来るので再現しない。
...
- **表紙に出す作品を4本以上にする**（§0。本人・2026-09-04「トップに出すのは3つくらいに」）。
  足すなら先にどれかを外す。**7本まで溜めたことがある**
- **表紙の `headlines` に新作を書く**（§0）。左の NEW WORKS 欄と重複する。
  **同じ作品が表紙に4回出たことがある**（2026-09-04）
- **NOW MAKING（制作中）の欄を復活させる**（§2。本人が消した）。戻すなら先に確認を取る
- **小物（`/bits`）に画像・本文・個別ページを足す。** 軽さがこの欄の仕様（§2）。
  重くすると「簡単に公開できる場所」でなくなって、また告知しないものが溜まる
- **ページ側で `/works/${ref}` を手で組み立てる**（§3。シナリオを指すと黙って404へ飛ぶ）
