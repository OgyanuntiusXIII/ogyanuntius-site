...
function launch(){if(!world?.mimi)return;countPlay();board.start();finalRecord=null;practice=false;resetMoments();bgm.currentTime=0;s=newFlight();s.region='北海道';s.nextStation=ROUTE[1];lastStation=0;accumulator=0;world.reset();flash=0;beginSequence('launch');updateUi();}
...
function simulate(dt){
 ...
 if(encounter){for(const r of world.rings)if(r.z+s.travel>=0){r.checked=true;r.obj.visible=false;}for(const b of world.blocks)if(b.z+s.travel>=-1){b.checked=true;b.obj.visible=false;b.destroyed=true;}}
 else{
  ...
  for(const b of world.blocks){if(s.obstacleGrace>0||b.suppressed)continue;if(!b.checked&&b.z+s.travel>=-1){b.checked=true;if(boxHit(s,b))hit(b);else if(nearMiss(s,b)){feedback('ニアミス');celebrate({kind:'near',label:'紙一重'});effect('near',{volume:.8});tone(690,.07);world.burst(s.x,s.y,0,12);}if(mode!=='playing')return;}}
 }
...
function updateUi(){
 ...
 $('mashCallout').hidden=!flight||!(e?.phase==='mash'||(!e&&s.guitar));$('mashCallout').classList.toggle('guitar',s.guitar&&!e);$('hud').classList.toggle('train-active',!!e);$('trainHud').hidden=mode!=='playing'||!e;
 if(e){
