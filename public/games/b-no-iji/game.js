...
import {startTrain,mashTrain,guitarSeconds,guitarRemaining,guitarLit,TRAIN_WARNING,TRAIN_MASH,TRAIN_MIN_HITS,TRAIN_MAX_SECONDS,GUITAR_MAX_EXTENSION} from './rush.js';
...
const bgm=$('bgm');let musicWanted=false;bgm.addEventListener('error',()=>console.warn('BGMの読み込みに失敗しました'));
// Volume levels (0..1) chosen in the pause menu. Effects go through the master gain; music gets its own gain node once
// audio starts, because iOS ignores the volume property of an audio element (the music played at full volume there).
const VOLUME_KEYS={bgm:'b-no-iji.volume.bgm',se:'b-no-iji.volume.se'},levels={bgm:1,se:1};
for(const k in VOLUME_KEYS){try{const raw=localStorage.getItem(VOLUME_KEYS[k]);if(raw!==null&&Number.isFinite(Number(raw)))levels[k]=clamp(Number(raw),0,1);}catch{}}
const musicLevel=()=>((s.guitar||(sequence?.kind==='guitar'&&cinema?.previewFlight.guitar))?.22:.48)*levels.bgm;
function applyVolumes(){const level=musicLevel();if(audio?.music){audio.music.gain.setTargetAtTime(level,audio.context.currentTime,.05);bgm.volume=1;}else bgm.volume=level;if(audio)audio.master.gain.setTargetAtTime(muted?0:.32*levels.se,audio.context.currentTime,.05);}
function syncMusic(){applyVolumes();const wanted=mode==='playing'||(mode==='cinematic'&&sequence&&['launch','guitar'].includes(sequence.kind)&&!sequenceHeld&&!sequence.done);bgm.muted=muted;if(Boolean(wanted)===musicWanted)return;musicWanted=Boolean(wanted);if(musicWanted)bgm.play().catch(()=>{musicWanted=false;});else bgm.pause();}
const held=[new Set(),new Set()],holdTime=[0,0];let audio;const soundSources=new Set();
function initAudio(){if(audio)return;try{const context=new AudioContext(),master=context.createGain();master.gain.value=muted?0:.32*levels.se;const limiter=context.createDynamicsCompressor();limiter.threshold.value=-10;limiter.knee.value=8;limiter.ratio.value=5;limiter.attack.value=.003;limiter.release.value=.15;master.connect(limiter).connect(context.destination);const length=context.sampleRate*2,buffer=context.createBuffer(1,length,context.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<length;i++)data[i]=(Math.random()*2-1)*.35;const noise=context.createBufferSource();noise.buffer=buffer;noise.loop=true;const filter=context.createBiquadFilter();filter.type='lowpass';filter.frequency.value=400;const wind=context.createGain();wind.gain.value=0;noise.connect(filter).connect(wind).connect(master);noise.start();const rotor=context.createOscillator();rotor.type='triangle';rotor.frequency.value=65;const hum=context.createGain();hum.gain.value=0;rotor.connect(hum).connect(master);rotor.start();audio={context,master,wind,filter,rotor,hum};try{const music=context.createGain();music.gain.value=musicLevel();context.createMediaElementSource(bgm).connect(music).connect(context.destination);audio.music=music;bgm.volume=1;}catch{}audio.guitar=new StrumAudio(context,master,soundSources);audio.effects=new Effects(context,master,soundSources);}catch{}}
...
function updateCinema(){if(!sequence)return;syncMusic();const k=sequence.kind,t=sequence.time;if(k==='goal'){$('cinemaTag').textContent='NISHI-OYAMA';$('cinemaText').textContent=t<1.2?'到着':'日本縦断';$('cinemaPause').textContent=sequenceHeld||sequence.done?'▶':'Ⅱ';$('cinemaPause').setAttribute('aria-label',sequence.done?'もう一度見る':sequenceHeld?'演出を再生':'演出を一時停止');$('cinemaProgress').style.width=t/SEQUENCES.goal.duration*100+'%';$('cinemaRetry').hidden=true;$('skip').textContent=scenePreview?'戻る':'結果へ';return;}$('cinemaTag').textContent=k==='launch'?'WAKKANAI / CATAPULT 01':k==='guitar'?(scenePreview?'PREVIEW':`${s.rings} RINGS`):'';$('cinemaText').textContent=k==='launch'?(t<.65?'発進準備':'出撃'):k==='guitar'?(t<GUITAR_DURATION?`ギター ${(GUITAR_DURATION-t).toFixed(1)}秒`:'通常飛行'):'';$('cinemaPause').textContent=sequenceHeld||sequence.done?'▶':'Ⅱ';$('cinemaPause').setAttribute('aria-label',sequence.done?'もう一度見る':sequenceHeld?'演出を再生':'演出を一時停止');$('cinemaProgress').style.width=t/SEQUENCES[k].duration*100+'%';$('cinemaRetry').hidden=scenePreview||k!=='stairs';$('skip').textContent=scenePreview?'戻る':k==='stairs'?'結果へ':'スキップ';}
...
function input(side,strength=1){
 ...
 if(s.guitar&&s.guitarStrums!==before){
  strum(s.guitarTime,s.guitarStrums-1);world.celebrations.emit('strum',s,reduced);$('guitarGain').textContent=s.guitarExtended>=GUITAR_MAX_EXTENSION-1e-9?'延長MAX':'+0.03秒';const timer=$('guitarTimer');timer.classList.remove('tapped');void timer.offsetWidth;timer.classList.add('tapped');updateUi();
 }else if(!s.guitar){tone(side<0?520:640,.05,0,.09*strength);effect('flap',{volume:.85*strength,pan:side*.6});}
...
function finish(clear){if(practice){showPractice('もう一回、ぶっ飛ばそう。');return;}s.km=Math.min(s.km,GOAL);stopEars(s);saveBest();setMode('result');$('resultTotals').hidden=true;const result=runResult(s);$('resultLabel').textContent=clear?'日本縦断':result.reason;$('result').classList.toggle('cleared',clear);$('clearMedal').hidden=!clear;$('resultKm').textContent=formatKm(s.km);$('resultMessage').textContent=result.place;renderResultMap(s.km);$('resultStats').textContent=`ベスト ${formatKm(best)} km　/　${formatTime(s.elapsed)}\n${s.station+1} / ${ROUTE.length}駅\nリング ${s.rings}個　/　最大 ×${s.maxCombo}\nニアミス ${s.nearMisses}回　/　破壊 ${s.destroyed}個`;$('shareX').href=shareUrl(s);finalRecord={...s};if(unranked){$('rankStatus').textContent='途中から出撃したので、ランキングは対象外';$('rankEntry').hidden=true;$('rankRetry').hidden=true;board.load().then(d=>showTotals(d.totals)).catch(()=>{});}else qualifyRank();effect('finish',{volume:.9});if(clear)effect('applause',{volume:1.55});}
async function qualifyRank(){
 ...
 try{const data=await board.qualify(record);if(finalRecord!==record)return;renderRanks(data.entries);showTotals(data.totals);$('rankStatus').textContent=data.rank?'':'今回は30位圏外';if(data.rank){$('rankPrompt').textContent=`${data.rank}位　名前を残す`;$('rankEntry').hidden=false;try{$('playerName').value=localStorage.getItem('b-no-iji.player')||'';}catch{}}}
...
// Everyone's flights so far: one row in D1, read back with the ranking.
function showTotals(t){if(!t||!Number.isFinite(Number(t.runs)))return;const el=$('resultTotals');el.replaceChildren('全プレイヤーの記録　');const runs=document.createElement('b');runs.textContent=Number(t.runs).toLocaleString('ja-JP');const km=document.createElement('b');km.textContent=formatKm(Number(t.metres)/1000);el.append(runs,'回の飛行 ／ 合計 ',km,' km');el.hidden=false;}
...
$('showRanks').onclick=async()=>{setMode('ranks');$('ranksStatus').textContent='読み込み中…';try{const data=await board.load();renderRanks(data.entries);showTotals(data.totals);}catch(e){$('ranksStatus').textContent=e.message;}};
...
$('mute').onclick=()=>{muted=!muted;applyVolumes();try{localStorage.setItem('mimicopter.3d.muted',muted?'1':'0');}catch{}updateMute();syncMusic();};function updateMute(){$('mute').textContent=muted?'♫̸':'♪';$('mute').setAttribute('aria-label',muted?'音を出す':'音を消す');}updateMute();
// Pause-menu sliders. A short sound confirms the effects level when the thumb is released.
for(const k in VOLUME_KEYS){const el=$(k+'Volume');el.value=String(Math.round(levels[k]*100));el.addEventListener('input',()=>{levels[k]=clamp(Number(el.value)/100,0,1);try{localStorage.setItem(VOLUME_KEYS[k],String(levels[k]));}catch{}applyVolumes();});if(k==='se')el.addEventListener('change',()=>{initAudio();audio?.context.resume();effect('ring',{volume:.7});});}
...
function updateUi(){
 ...
 if(e){
  ...
  $('trainDetail').textContent=e.phase==='warning'?'電車が来る':e.phase==='mash'?`${e.hits<TRAIN_MIN_HITS?'あと'+(TRAIN_MIN_HITS-e.hits)+'打':guitarSeconds(e.hits)>=TRAIN_MAX_SECONDS?'最大まで来た！':'まだいける！'}　/　ギター ${guitarSeconds(e.hits).toFixed(1)}秒${guitarSeconds(e.hits)>=TRAIN_MAX_SECONDS?'（最大）':''}`:`ギター ${s.guitarDuration.toFixed(1)}秒`;
