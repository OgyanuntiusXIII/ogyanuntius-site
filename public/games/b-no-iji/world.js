...
import {ringAt,obstacleAt} from './course.js';
...
export class World{
 constructor(canvas){
  ...
  this.tiles=[];for(let i=0;i<7;i++)this.tiles.push(this.makeTile(i));this.landmarks=DISTRICTS.map((d,i)=>{const g=makeLandmark(i),mountain=['volcano','kaimon','fuji','lake'].includes(d.kind);if(mountain)g.scale.setScalar(d.kind==='volcano'?.72:.85);this.scene.add(g);return{g,distance:ROUTE[i].km/KM_PER_UNIT+120,x:(i%2?1:-1)*(mountain?35:28)};});
  ...
  this.rings=[];const ringGeo=new T.TorusGeometry(RING_RADIUS,.13,10,48);const outerGeo=new T.TorusGeometry(RING_RADIUS+.22,.027,4,48);for(let i=0;i<10;i++){const g=new T.Group();g.add(new T.Mesh(ringGeo,suica));g.add(new T.Mesh(outerGeo,white));for(let j=0;j<4;j++){const a=j*Math.PI/2;const gem=mesh(new T.OctahedronGeometry(.17),white,Math.cos(a)*RING_RADIUS,Math.sin(a)*RING_RADIUS,0);g.add(gem);}this.scene.add(g);this.rings.push({obj:g,z:0,x:0,y:5,checked:false});}
  this.blocks=[];for(let i=0;i<6;i++){const g=new T.Group();g.add(mesh(boxGeo,red,0,0,0,3,5,2));for(let y=-1.9;y<2.5;y+=1.25){const stripe=mesh(boxGeo,white,0,y,1.015,3.01,.32,.02);stripe.rotation.z=.12;g.add(stripe);}this.scene.add(g);this.blocks.push({obj:g,z:0,x:0,w:3,h:5,checked:false});}
 ...
 reset(){this.celebrations.reset();this.express.visible=false;this.flightCamera.reset();for(const d of this.debris??[])this.scene.remove(d.obj);this.debris=[];this.tiles.forEach((t,i)=>{t.base=-i*160;t.district=-1;});this.rings.forEach((r,i)=>this.placeRing(r,i));this.blocks.forEach((b,i)=>this.placeBlock(b,i));this.lastGate='';this.ringSerial=10;this.blockSerial=6;for(const p of this.particleData)p.life=0;}
 placeRing(r,i){Object.assign(r,ringAt(i));r.checked=false;r.obj.visible=true;r.obj.position.set(r.x,r.y,r.z);}
 placeBlock(b,i){Object.assign(b,obstacleAt(i));b.checked=false;b.destroyed=false;b.suppressed=false;b.obj.visible=true;b.obj.position.set(b.x,b.baseY+b.h/2,b.z);b.obj.scale.set(b.w/3,b.h/5,1);}
 ...
 update(dt,s,mode,time,reduced=false){
  ...
  for(const b of this.blocks){
   ...
   if(s.obstacleGrace>0){b.suppressed=true;b.obj.visible=false;}
   else if(b.suppressed&&z<-240){b.suppressed=false;b.checked=false;b.obj.visible=!b.destroyed;}
  }
